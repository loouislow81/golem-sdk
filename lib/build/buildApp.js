...
function normalizeAppName(appName, url) {
  ...
  hash.update(url);
  var postFixHash = hash.digest('hex').substring(0, 6); // 6 chars hash string
...
function buildApp(src, dest, options, callback) {
  ...
  copy(src, dest, function (error) {
    ...
    _fs2.default.writeFileSync(_path2.default.join(dest, '/golem.json'), JSON.stringify(appArgs));
